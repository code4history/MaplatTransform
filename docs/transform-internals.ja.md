...
1. **ウエイトバッファ** — `weight_buffer` キーは必須のまま残ります（MaplatTin 2.00704 以降は常に `{}` を書き出す）が、読み込み側は 2.00704 以降その値を無視します。
...
3. **三角形補間** — ヒットした三角形は `transformTinArr()` で純アフィン補間（バリセントリック座標）を行います。`weight_buffer` は 2.00704 以降読み込みません。
