...
> **ADR-0013（英語正・日本語ミラー）の適用外**: 本書には英語版を用意していません。
> ADR-0013 が対象とするのは公開リポジトリの README と Wiki の全ページであり、
> 本書はそのどちらでもない `docs/` 配下の内部文書です。加えて、利用者向けの
> 説明ではなく未実施の改修に関する開発者向けの計画書であるため、英訳は行わない
> と決定しました（2026-08-10）。
