/**
 * MaplatTransform V2/V3 Visualization Demo
 *
 ...
 */
function getEnvelopeCorners(compiled: Compiled, transform: Transform, dir: "forw" | "bakw"): Pos[] {
  const forwCorners = getEnvelopeForw(compiled);
  if (dir === "forw") return forwCorners;
  // Target 側: 各隅を forward 変換して Target 座標を得る
  return forwCorners
    .map(pt => transform.transform(pt, false, true) as Pos | false)
...
function drawTriangles(ctx: CanvasRenderingContext2D, transform: Transform, vp: Viewport, dir: "forw" | "bakw") {
  const tins = transform.tins?.[dir];
  if (!tins) return;
  ctx.save();
...
function vizRays(
  ...
  const { transform, compiled } = state;
  ...
  const envF = getEnvelopeCorners(compiled, transform, "forw");
  const envB = getEnvelopeCorners(compiled, transform, "bakw");
  ...
  for (const [ctx, vp, dir, env, z] of [
    ...
    drawTriangles(ctx, transform, vp, dir);
...
function vizGrid(
  ...
  const gridB: (Pos | null)[][] = ys.map(y =>
    xs.map(x => { const r = transform.transform([x, y], false, true); return r ? r as Pos : null; })
  );
  ...
  drawTriangles(ctxF, transform, vpF, "forw");
  ...
  drawTriangles(ctxB, transform, vpB, "bakw");
...
function vizRoundtrip(
  ...
  const cells: Cell[][] = ys.map(y =>
    xs.map(x => {
      const p2 = transform.transform([x, y], false, true);
      if (!p2) return { err: -1, bakwPt: null };
      let p3: number[] | false;
      try { p3 = transform.transform(p2, true, true); } catch { p3 = false; }
...
async function loadState(mapConfig: MapConfig, ver: FormatVer): Promise<LoadedState> {
  const filePath = `./compiled/${mapConfig.key}_${ver}.json`;
  const compiled: Compiled = await fetch(filePath).then(r => {
    if (!r.ok) throw new Error(`${r.status} ${r.url}`);
    return r.json();
  });
  const transform = new Transform();
  transform.setCompiled(compiled);
  return { transform, compiled, mapLabel: mapConfig.label, ver };
}
...
async function main() {
  ...
  canvasForwEl.addEventListener("click", (e: MouseEvent) => {
    ...
    const result = currentState.transform.transform(dataPt, false, true);
  ...
  canvasBakwEl.addEventListener("click", (e: MouseEvent) => {
    ...
    const result = currentState.transform.transform(dataPt, true, true);
